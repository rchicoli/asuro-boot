...
/* onboard LED is used to indicate, that the bootloader was entered (4x flashing) ADABOOT ... BBR*/
...
void flash_led(uint8_t count)
{
	...
	for(l = 0; l < (F_CPU / 50); ++l);		    // pause ADA BOOT BBR 

    for (i = 0; i < count; ++i) {
		LED_PORT |= _BV(LED);					// LED on
		for(l = 0; l < (F_CPU / 900); ++l);		// delay NGvalue was 1000 for both loops - BBR
		LED_PORT &= ~_BV(LED);					// LED off
		for(l = 0; l < (F_CPU / 125); ++l);		// delay asymmteric for ADA BOOT BBR 
	}
